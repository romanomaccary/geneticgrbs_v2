...
class LC(object):
    ...
    def __init__(self, q,a,alpha, k, t_0,t_min=+0.1, t_max=1000, res=0.256, 
                 ...
                 n_cut=None, instrument='batse', verbose=False,alpha_bpl=0.5, beta_bpl=1.5, F_break=1e-6, F_min=1e-10): #New parameters of the BPL count distrib
    ...
    def generate_LC_from_sde(self,q,a,alpha,k,t_0,alpha_bpl,beta_bpl,F_break,F_min,norm_A):
        ...
        def generale_lc_from_solution_SDE(q,a,alpha,k,t_0,alpha_bpl,beta_bpl,F_break,F_min,norm_A,times):
            norm_B = generate_peak_counts(self.generated_fluence, self.k_values)
            norm_B = np.float64(norm_B)
            #print("norm=",norm_B)
        ...
        self._rates = generale_lc_from_solution_SDE(q,a,alpha,k,t_0,alpha_bpl,beta_bpl,F_break,F_min,norm_A,self._times)
        ...
        if self._instrument == 'batse' or self._instrument == 'fermi':
            ...
            if np.any(self._modelbkg > 1e17):
                print('HIGH LAMBDA')
                self.plot_lc=np.random.normal(loc=self._modelbkg, scale=np.sqrt(self._modelbkg))
            else:
